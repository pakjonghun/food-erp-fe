...
import Toolbar from './SubsidiaryToolbar';
...
import { useUpdateSubsidiary } from '@/graphql/hooks/subsidiary/update';
...
const SubsidiaryGrid: FC<Props> = ({ rows, loading }) => {
  const apiRef = useGridApiRef();
  const [update, { loading: updating }] = useUpdateSubsidiary();
  ...
  const handleCellUpdate = async (newRow: Products, oldRow: any) => {
    try {
      ...
      return result.data?.updateSubsidiary ?? oldRow;
    } catch (err: unknown) {
