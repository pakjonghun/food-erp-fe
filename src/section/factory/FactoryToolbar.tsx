...
import ProductUpload from './FactoryUpload';
...
import { useRemoveManyFactory } from '@/graphql/hooks/factory/removeMany';
...
const FactoryToolbar: FC<Props> = ({ column }) => {
  const [keyword, setKeyword] = useState('');
  const delayText = useTextDebounce({ keyword });
  const handleChangeTarget = (target: string) => factoryTarget(target);
  const handleChangeFactoryKeyword = (target: string) => factoryKeyword(target);
  const searchCount = useReactiveVar(factoryCount);
  const target = useReactiveVar(factoryTarget);

  const [remove, { loading }] = useRemoveManyFactory();
  ...
  useEffect(() => {
    handleChangeFactoryKeyword(delayText);
  }, [delayText]);
