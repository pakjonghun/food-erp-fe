...
import Toolbar from './ClientToolbar';
import EmptyRow from '@/components/dataGrid/EmptyRow';
import { Clients, Products } from '@/graphql/codegen/graphql';
import { useSnack } from '@/context/snackContext/SnackProvider';
import useGetColumn from './useGetColumn';
import { ApolloError } from '@apollo/client';
import { useUpdateClient } from '@/graphql/hooks/client/update';
...
const ClientGrid: FC<Props> = ({ rows, loading }) => {
  const apiRef = useGridApiRef();
  const [update, { loading: updating }] = useUpdateClient();

  const [editField, setEditField] = useState<keyof Clients | null>('name');
  ...
  const handleCellUpdate = async (newRow: Products, oldRow: any) => {
    try {
      ...
      return result.data?.updateClient ?? oldRow;
    } catch (err: unknown) {
  ...
  const handleCellEditStart = (params: GridCellEditStartParams) => {
    const field = params.field as keyof Clients;
    setEditField(field);
  };
